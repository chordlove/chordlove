...
ChordBox.prototype.draw = function() {
  ...
  // Draw frets
  var fretDiff = this.position === 0 ? 0 : this.position - this.position_text - 1;
  for (var i = 0; i < this.num_frets + 1; ++i) {
    this.paper.vexLine(this.x, this.y + (fret_spacing * i),
      this.x + (spacing * (this.num_strings - 1)),
      this.y + (fret_spacing * i));
    if (i > 0) {
      var realFret = i + fretDiff;
      if (realFret === 3 || realFret === 5 || realFret === 7 || realFret === 9 
          || realFret === 15 || realFret === 17 || realFret === 19) {
        drawDot(2.5, i, this);
      } else if (realFret === 12) {
        drawDot(1.5, i, this);
        drawDot(3.5, i, this);
  ...
  function drawDot(stringPosition, index, global) {
    global.paper.circle(
        global.x + spacing * stringPosition, 
        global.y + (fret_spacing * ( index - 0.5)), global.metrics.dot_radius)
        .attr({"fill": "transparent", "stroke-width" : 1, "stroke" : "#666666"});
  }
  ...
  for (var i = 0; i < tuning.length; ++i) {
    var t = this.paper.text(
      ...
      tuning[i]);
    t.attr({"font-size": this.metrics.font_size, 'font-family' : 'DejaVuSerifBook'});
  }
